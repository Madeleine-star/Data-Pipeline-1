...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:output method='text' indent='yes'/><!--Specify the type of file for the output, we set it has text because the json isn't allowing here and the default is xml-->
	<!--
...
-->
	<xsl:template match="/"><!--We declared in first match="/" because we want the template is apply to all the xml hierarchie because the / symbolize the root element-->
...
<xsl:apply-templates select="Root/Customers/Customer"/>
	...
	<xsl:template match="Root/Customers/Customer"><!--Here we declare a second object which will contains the value of the array, for each customer a new object is created-->
...
<xsl:if test="position() != last()">,</xsl:if> <!--Because in a JSON file the last element must don't have a comma after the curlybracket we used the if condition to test the iteration position and 
